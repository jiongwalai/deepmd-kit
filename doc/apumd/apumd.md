...
APUMD stands for non-von Neumann molecular dynamics.
...
Any user can follow two consecutive steps to run molecular dynamics (MD) on the proposed APUMD computer, which has been released online: (i) to train a machine learning (ML) model that can decently reproduce the potential energy surface (PES); and (ii) to deploy the trained ML model on the proposed APUMD computer, then run MD there to obtain the atomistic trajectories.
...
If you just want to simply test the training function, you can use the example in the `$deepmd_source_dir/examples/apumd` directory. If you want to fully experience training and running MD functions, you can download the complete example from the [website](https://github.com/LiuGroupHNU/apumd-example).
...
```bash
cp -r $deepmd_source_dir/examples/apumd/train/train_cnn.json train_cnn.json
cp -r $deepmd_source_dir/examples/apumd/train/train_qnn.json train_qnn.json
```
...
The "apumd" section is defined as
...
Multiple versions of the apumd model correspond to different network structures. `apumd-v0` and `apumd-v1` differ in the following ways:

1. `apumd-v0` and `apumd-v1` use the `se_a` descriptor and `se_atten` descriptor, respectively
1. `apumd-v0` has 1 set of parameters for each element and supports up to 4 element types. `apumd-v1` shares 1 set of parameters for each element and supports up to 31 types.
1. `apumd-v0` distinguishes between neighboring atoms, so `sel` is a list of integers. `apumd-v1` does not distinguish between neighboring atoms, so `sel` is an integer.
...
```bash
# step1: train CNN
dp train-apumd train_cnn.json -s s1
# step2: train QNN
dp train-apumd train_qnn.json -s s2
```

After the training process, you will get two folders: `apumd_cnn` and `apumd_qnn`. The `apumd_cnn` contains the model after continuous neural network (CNN) training. The `apumd_qnn` contains the model after quantized neural network (QNN) training. The binary file `apumd_qnn/model.pb` is the model file that is used to perform APUMD in the server [http://apumd.picp.vip].

You can also restart the CNN training from the path prefix of checkpoint files (`apumd_cnn/model.ckpt`) by

```bash
dp train-apumd train_cnn.json -r apumd_cnn/model.ckpt -s s1
```

You can also initialize the CNN model and train it by

```bash
mv apumd_cnn apumd_cnn_bck
cp train_cnn.json train_cnn2.json
# please edit train_cnn2.json
dp train-apumd train_cnn2.json -s s1 -i apumd_cnn_bck/model.ckpt
```
...
```bash
mkdir test
dp test -m ./apumd_qnn/frozen_model.pb -s path/to/system -d ./test/detail -n 99999 -l test/output.log
```
...
# Running MD in Apumd website

After CNN and QNN training, you can upload the ML model to our online APUMD system and run MD there.

## Account application

The server website of APUMD is available at http://apumd.picp.vip. You can visit the URL and enter the login interface.
