...
#include "device.h"
#include "env_mat_apumd.h"
#include "fmt_nlist.h"
#include "neighbor_list.h"
#include "prod_env_mat_apumd.h"

class TestEnvMatAApumd : public ::testing::Test {
 protected:
...
class TestEnvMatAApumdShortSel : public ::testing::Test {
 protected:
  std::vector<double> posi = {12.83, 2.56, 2.18, 12.09, 2.87, 2.74,
                              00.25, 3.32, 1.68, 3.36,  3.00, 1.81,
                              3.51,  2.51, 2.60, 4.27,  3.22, 1.56};
...
/*  env_mat_a_apumd_quantize_cpu is not same as env_mat_a.
remove some tests:
TEST_F(TestEnvMatAApumd, orig_cpy)
TEST_F(TestEnvMatAApumd, orig_pbc)
TEST_F(TestEnvMatAApumd, orig_cpy_equal_pbc)
TEST_F(TestEnvMatAApumd, orig_cpy_num_deriv)
*/

TEST_F(TestEnvMatAApumd, cpu) {
  ...
  for (int ii = 0; ii < nloc; ++ii) {
    ...
    EXPECT_EQ(ret, -1);
    deepmd::env_mat_a_apumd_quantize_cpu<double>(
...
/*  env_mat_a_apumd_quantize_cpu is not same as env_mat_a.
remove some tests:
TEST_F(TestEnvMatAApumd, cpu_equal_orig_cpy)
TEST_F(TestEnvMatAApumd, cpu_num_deriv)
TEST_F(TestEnvMatAApumdShortSel, orig_cpy)
TEST_F(TestEnvMatAApumdShortSel, orig_pbc)
*/

TEST_F(TestEnvMatAApumdShortSel, cpu) {
  std::vector<int> fmt_nlist_a, fmt_nlist_r;
  std::vector<double> env, env_deriv, rij_a;
  bool pbc = false;
  for (int ii = 0; ii < nloc; ++ii) {
...
TEST_F(TestEnvMatAApumd, prod_cpu) {
  ...
  deepmd::prod_env_mat_a_apumd_quantize_cpu(
...
TEST_F(TestEnvMatAApumd, prod_cpu_equal_cpu) {
  EXPECT_EQ(nlist_r_cpy.size(), nloc);
  int tot_nnei = 0;
  int max_nbor_size = 0;
  for (int ii = 0; ii < nlist_a_cpy.size(); ++ii) {
