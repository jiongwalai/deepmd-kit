...
class FltApumdOp : public OpKernel {
 public:
  /// Constructor.
  explicit FltApumdOp(OpKernelConstruction* context) : OpKernel(context) {};
...
#define REGISTER_CPU(T)                                           \
  REGISTER_KERNEL_BUILDER(                                        \
      Name("FltApumd").Device(DEVICE_CPU).TypeConstraint<T>("T"), \
      FltApumdOp<CPUDevice, T>);
REGISTER_CPU(float);
REGISTER_CPU(double);
