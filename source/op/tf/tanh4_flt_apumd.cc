...
// New Activation Function of APUMD
// --------------------------------------------------------------------
/*
...
class Tanh4FltApumdOp : public OpKernel {
 public:
  /// Constructor.
  explicit Tanh4FltApumdOp(OpKernelConstruction* context) : OpKernel(context) {
...
#define REGISTER_CPU(T)                                                \
  REGISTER_KERNEL_BUILDER(                                             \
      Name("Tanh4FltApumd").Device(DEVICE_CPU).TypeConstraint<T>("T"), \
      Tanh4FltApumdOp<CPUDevice, T>);
REGISTER_CPU(float);
REGISTER_CPU(double);
