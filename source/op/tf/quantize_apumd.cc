...
// Quantization Operator of APUMD
// --------------------------------------------------------------------
/*
...
class QuantizeApumdOp : public OpKernel {
 public:
  /// Constructor.
  explicit QuantizeApumdOp(OpKernelConstruction* context) : OpKernel(context) {
...
#define REGISTER_CPU(T)                                                \
  REGISTER_KERNEL_BUILDER(                                             \
      Name("QuantizeApumd").Device(DEVICE_CPU).TypeConstraint<T>("T"), \
      QuantizeApumdOp<CPUDevice, T>);
REGISTER_CPU(float);
REGISTER_CPU(double);
