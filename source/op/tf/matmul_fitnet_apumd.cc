...
class MatmulFitnetApumdOp : public OpKernel {
 public:
  /// Constructor.
  explicit MatmulFitnetApumdOp(OpKernelConstruction* context)
      : OpKernel(context) {
...
#define REGISTER_CPU(T)                                                    \
  REGISTER_KERNEL_BUILDER(                                                 \
      Name("MatmulFitnetApumd").Device(DEVICE_CPU).TypeConstraint<T>("T"), \
      MatmulFitnetApumdOp<CPUDevice, T>);
REGISTER_CPU(float);
REGISTER_CPU(double);
