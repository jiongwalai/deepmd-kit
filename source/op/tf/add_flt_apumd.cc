...
class AddFltApumdOp : public OpKernel {
 public:
  /// Constructor.
  explicit AddFltApumdOp(OpKernelConstruction* context) : OpKernel(context) {};
...
#define REGISTER_CPU(T)                                              \
  REGISTER_KERNEL_BUILDER(                                           \
      Name("AddFltApumd").Device(DEVICE_CPU).TypeConstraint<T>("T"), \
      AddFltApumdOp<CPUDevice, T>);
REGISTER_CPU(float);
REGISTER_CPU(double);
