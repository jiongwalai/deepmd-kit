...
class DotmulFltApumdOp : public OpKernel {
 public:
  /// Constructor.
  explicit DotmulFltApumdOp(OpKernelConstruction* context)
      : OpKernel(context) {};
...
#define REGISTER_CPU(T)                                                 \
  REGISTER_KERNEL_BUILDER(                                              \
      Name("DotmulFltApumd").Device(DEVICE_CPU).TypeConstraint<T>("T"), \
      DotmulFltApumdOp<CPUDevice, T>);
REGISTER_CPU(float);
REGISTER_CPU(double);
