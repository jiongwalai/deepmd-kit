...
REGISTER_OP("ProdEnvMatAApumdQuantize")
...
// ProdEnvMatAMixApumd
REGISTER_OP("ProdEnvMatAMixApumdQuantize")
    .Attr("T: {float, double} = DT_DOUBLE")
    .Input("coord: T")       // atomic coordinates
    .Input("type: int32")    // atomic type
    .Input("natoms: int32")  // local atomic number; each type atomic number
...
/*
//==================================================
  ProdEnvMatAApumdQuantizeOp
//==================================================
*/

template <typename Device, typename FPTYPE>
class ProdEnvMatAApumdQuantizeOp : public OpKernel {
 public:
  explicit ProdEnvMatAApumdQuantizeOp(OpKernelConstruction* context)
      : OpKernel(context) {
    float nloc_f, nall_f;
...
/*
//==================================================
  ProdEnvMatAMixApumdQuantizeOp
//==================================================
*/

template <typename Device, typename FPTYPE>
class ProdEnvMatAMixApumdQuantizeOp : public OpKernel {
 public:
  explicit ProdEnvMatAMixApumdQuantizeOp(OpKernelConstruction* context)
      : OpKernel(context) {
    float nloc_f, nall_f;
    OP_REQUIRES_OK(context, context->GetAttr("rcut_a", &rcut_a));
    OP_REQUIRES_OK(context, context->GetAttr("rcut_r", &rcut_r));
...
#define REGISTER_CPU(T)                                              \
  REGISTER_KERNEL_BUILDER(Name("ProdEnvMatAApumdQuantize")           \
                              .Device(DEVICE_CPU)                    \
                              .TypeConstraint<T>("T"),               \
                          ProdEnvMatAApumdQuantizeOp<CPUDevice, T>); \
  REGISTER_KERNEL_BUILDER(Name("ProdEnvMatAMixApumdQuantize")        \
                              .Device(DEVICE_CPU)                    \
                              .TypeConstraint<T>("T"),               \
                          ProdEnvMatAMixApumdQuantizeOp<CPUDevice, T>);
