...
REGISTER_OP("MatmulFlt2fixApumd")
...
class MatmulFlt2fixApumdOp : public OpKernel {
 public:
  /// Constructor.
  explicit MatmulFlt2fixApumdOp(OpKernelConstruction* context)
      : OpKernel(context) {
 ...
 private:
  int nbit;
};  // MatmulFlt2fixApumdOp

#define REGISTER_CPU(T)                                                     \
  REGISTER_KERNEL_BUILDER(                                                  \
      Name("MatmulFlt2fixApumd").Device(DEVICE_CPU).TypeConstraint<T>("T"), \
      MatmulFlt2fixApumdOp<CPUDevice, T>);
REGISTER_CPU(float);
REGISTER_CPU(double);
