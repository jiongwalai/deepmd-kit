...
void mul_flt_apumd(T& y, T x1, T x2);

template <class T>  // float and double
void add_flt_apumd(T& y, T x1, T x2);

//- register the operator
// prec = 2^n, so it doesn't need to match `T`
REGISTER_OP("MapFltApumd")
...
class MapFltApumdOp : public OpKernel {
 public:
  /// Constructor.
  explicit MapFltApumdOp(OpKernelConstruction* context) : OpKernel(context) {}
    ...
    for (ss = S - 1; ss >= 0; ss--) {
      ...
      for (ii = 0; ii < N * D; ii++) {
        ...
        for (jj = 0; jj < M; jj++) {
          ...
          mul_flt_apumd(ytmp, a, xx);
          add_flt_apumd(ytmp, b, ytmp);
          mul_flt_apumd(ytmp, ytmp, xx);
          add_flt_apumd(ytmp, c, ytmp);
          mul_flt_apumd(ytmp, ytmp, xx);
          add_flt_apumd(ytmp, d, ytmp);
          y[ii * M + jj] = ytmp;
        }  // jj
      }  // ii
    }  // ss
...
#define REGISTER_CPU(T)                                              \
  REGISTER_KERNEL_BUILDER(                                           \
      Name("MapFltApumd").Device(DEVICE_CPU).TypeConstraint<T>("T"), \
      MapFltApumdOp<CPUDevice, T>);
REGISTER_CPU(float);
REGISTER_CPU(double);
