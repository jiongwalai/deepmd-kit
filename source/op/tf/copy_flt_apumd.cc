...
class CopyFltApumdOp : public OpKernel {
 public:
  /// Constructor.
  explicit CopyFltApumdOp(OpKernelConstruction* context) : OpKernel(context) {};
...
#define REGISTER_CPU(T)                                               \
  REGISTER_KERNEL_BUILDER(                                            \
      Name("CopyFltApumd").Device(DEVICE_CPU).TypeConstraint<T>("T"), \
      CopyFltApumdOp<CPUDevice, T>);
REGISTER_CPU(float);
REGISTER_CPU(double);
