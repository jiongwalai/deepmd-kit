...
class MulFltApumdOp : public OpKernel {
 public:
  /// Constructor.
  explicit MulFltApumdOp(OpKernelConstruction* context) : OpKernel(context) {};
  ...
  void Compute(OpKernelContext* context) override {
    ...
    if (M1 == M2) {
      for (ii = 0; ii < H * N * M2; ii++) {
        mul_flt_apumd(y[ii], x[ii], w[ii]);
      }
    } else {
      for (ii = 0; ii < H * N; ii++) {
        for (jj = 0; jj < M2; jj++) {
          mul_flt_apumd(y[ii * M2 + jj], x[ii], w[ii * M2 + jj]);
        }
      }
    }
...
#define REGISTER_CPU(T)                                              \
  REGISTER_KERNEL_BUILDER(                                           \
      Name("MulFltApumd").Device(DEVICE_CPU).TypeConstraint<T>("T"), \
      MulFltApumdOp<CPUDevice, T>);
REGISTER_CPU(float);
REGISTER_CPU(double);
