...
def assert_apumd_close(actual: np.ndarray | float, expected: np.ndarray | float) -> None:
    np.testing.assert_allclose(actual, expected, rtol=0.0, atol=1e-5)


class TestApumdEntrypointsV0(tf.test.TestCase):
    @pytest.mark.run(order=0)
    def test_mapt_cnn_v0(self) -> None:
        config_file = str(tests_path / "apumd" / "ref" / "config_v0_cnn.npy")
        weight_file = str(tests_path / "apumd" / "ref" / "weight_v0_cnn.npy")
        map_file = str(tests_path / "apumd" / "out" / "map_v0_cnn.npy")
        # mapt
        ...
        assert_apumd_close(pred, ref_dout)
        #
        N = 4
    ...
    def test_model_qnn_v0(self) -> None:
        ...
        # open APUMD
        jdata_cf = jdata_deepmd_input_v0["apumd"]
        jdata_cf["config_file"] = str(
            tests_path / "apumd" / "ref" / "config_v0_cnn.npy"
        )
        jdata_cf["weight_file"] = str(
            tests_path / "apumd" / "ref" / "weight_v0_cnn.npy"
        )
        jdata_cf["map_file"] = str(tests_path / "apumd" / "out" / "map_v0_cnn.npy")
        jdata_cf["enable"] = True
        apumd_cfg.init_from_jdata(jdata_cf)
        apumd_cfg.init_train_mode("qnn")
        # build trainer
        ntype = apumd_cfg.dscp["ntype"]
        jdata = apumd_cfg.get_deepmd_jdata()
        ...
        # get feed_dict
        crd_file = str(tests_path / "apumd" / "ref" / "coord.npy")
        box_file = str(tests_path / "apumd" / "ref" / "box.npy")
        type_file = str(tests_path / "apumd" / "ref" / "type.raw")
        ...
        assert_apumd_close(pred[idx], ref_dout)
        # o_rmat
    ...
    def tearDown(self) -> None:
        # close
        apumd_cfg.enable = False
        tf.reset_default_graph()


class TestApumdEntrypointsV1(tf.test.TestCase):
    @pytest.mark.run(order=0)
    def test_mapt_cnn_v1(self) -> None:
        config_file = str(tests_path / "apumd" / "ref" / "config_v1_cnn.npy")
        weight_file = str(tests_path / "apumd" / "ref" / "weight_v1_cnn.npy")
        map_file = str(tests_path / "apumd" / "out" / "map_v1_cnn.npy")
        # mapt
        mapObj = MapTable(config_file, weight_file, map_file)
        mapObj.Gs_Gt_mode = 0
        mapt = mapObj.build_map()
    ...
    def test_model_qnn_v1(self) -> None:
        ...
        tf.reset_default_graph()
        # open APUMD
        jdata_cf = jdata_deepmd_input_v1["apumd"]
        jdata_cf["config_file"] = str(
            tests_path / "apumd" / "ref" / "config_v1_cnn.npy"
        )
        jdata_cf["weight_file"] = str(
            tests_path / "apumd" / "ref" / "weight_v1_cnn.npy"
        )
        jdata_cf["map_file"] = str(tests_path / "apumd" / "out" / "map_v1_cnn.npy")
        jdata_cf["enable"] = True
        apumd_cfg.init_from_jdata(jdata_cf)
        apumd_cfg.init_train_mode("qnn")
        # build trainer
        ...
        # test freeze
        sess = self.cached_session().__enter__()
        weight_file1 = str(tests_path / "apumd" / "ref" / "weight_v1_cnn.npy")
        weight_file2 = str(tests_path / "apumd" / "out" / "weight_v1_qnn.npy")
    ...
    def test_wrap_qnn_v1(self) -> None:
        ...
        tf.reset_default_graph()
        jdata = {}
        jdata["apumd_config"] = str(tests_path / "apumd" / "ref" / "config_v1_cnn.npy")
        jdata["apumd_weight"] = str(tests_path / "apumd" / "ref" / "weight_v1_cnn.npy")
        jdata["apumd_map"] = str(tests_path / "apumd" / "out" / "map_v1_cnn.npy")
        jdata["apumd_model"] = str(tests_path / "apumd" / "out" / "model_v1_qnn.pb")
        wrap(**jdata)
        # test
        data = FioBin().load(jdata["apumd_model"])
