...
class Wrap:
            ...
            for tt in range(ntype_max):
                for tt2 in range(ntype_max):
                    if (tt < ntype) and (tt2 < ntype):
                        ...
                        feed_dict = {
                            t_x: np.ones([1, 1], dtype=np.float32) * 0.0,
                            t_table: mi,
                            t_table_grad: mi * 0.0,
                            t_table_info: cfgs,
                        }
                        ...
                        cfgs = np.array([np.float64(v) for vs in cfgs for v in vs])
                        feed_dict = {
                            t_x: np.ones([1, 1], dtype=np.float32) * si,
                            t_table: mi,
                            t_table_grad: mi * 0.0,
                            t_table_info: cfgs,
                        }
                        ...
                        gsi = np.zeros(M1, dtype=np.float32)
                    for ii in range(M1):
                        GSs.extend(
    ...
    def wrap_weight(self, weight, NBIT_DISP, NBIT_WEIGHT):
        ...
        sh = weight.shape
        nr, nc = sh[0], sh[1]
        nrs = np.zeros(nr, dtype=np.float32)
        ncs = np.zeros(nc, dtype=np.float32)
        wrs = np.zeros([nr, nc], dtype=np.float32)
        wcs = np.zeros([nr, nc], dtype=np.float32)
        e = Encode()
        # row
        for ii in range(nr):
            wi = weight[ii, :]
    ...
    def wrap_lut(self):
        ...
        d2 = np.zeros([ntype_max, 4], dtype=np.float32)
        ...
        # gtt
        d = maps["gt"]
        d2 = np.zeros([ntype_max**2, M1], dtype=np.float32)
        for ii in range(ntype):
        ...
        nd = w.shape[1]
        d2 = np.zeros([ntype_max, nd], dtype=np.float32)
        for ii in range(ntype):
            _d = d[ii]
