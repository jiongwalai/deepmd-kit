...
class MapTable:
        ...
        for key in dic_map.keys():
            ...
            if isinstance(val, list):
                ...
                for ii in range(len(val)):
                    ...
                    dat_i = np.zeros([n, nc], dtype=np.float32)
                    for kk in range(n):
                        xk = x[kk]
                        for cfg in cfgs:
                            x0, x1, dx, N0, N1 = cfg
    ...
    def run_u2s(self):
        ...
        u = N2 * np.reshape(np.arange(0, N + 1, dtype=np.float32) / N, [-1, 1])
        res_lst = run_sess(sess, vals, feed_dict={dic_ph["u"]: u})
        res_dic = dict(zip(keys, res_lst))

        u2 = N2 * np.reshape(np.arange(0, N * 16 + 1, dtype=np.float32) / (N * 16), [-1, 1])
    ...
    def run_s2g(self):
        ...
        s = N2 * np.reshape(np.arange(0, N + 1, dtype=np.float32) / N, [-1, 1]) + smin_
        res_lst = run_sess(sess, vals, feed_dict={dic_ph["s"]: s})
        res_dic = dict(zip(keys, res_lst))

        s2 = N2 * np.reshape(np.arange(0, N * 16 + 1, dtype=np.float32) / (N * 16), [-1, 1]) + smin_
