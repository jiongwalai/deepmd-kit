...
class Wrap:
    ...
    the model file can be use to run the APUMD with lammps
    the pair style need set as:

    .. code-block:: lammps

        pair_style apu model.pb
        pair_coeff * *
    ...
    config_file
        input file name
        an .npy file containing the configuration information of APUMD model
    weight_file
        input file name
        an .npy file containing the weights of APUMD model
    map_file
        input file name
        an .npy file containing the mapping tables of APUMD model
    model_file
        output file name
        an .pb file containing the model using in the APUMD
    ...
    def __init__(
        ...
        if loc_version == 1:
            jdata = copy.deepcopy(jdata_deepmd_input_v1_ni256["apumd"])
        else:
            jdata = copy.deepcopy(jdata_deepmd_input_v0["apumd"])
        jdata["device"] = "vu13p" if loc_nstdm == 32 else "vu9p"
    ...
    def wrap(self) -> None:
        ...
        for ii in range(len(datas)):
            ...
            if jdata_sys["debug"]:
                ...
                FioTxt().save(f"apumd/wrap/h{k}.txt", d)
            datas[ii] = d
        ...
        log.info("APUMD: finish wrapping model file")
    ...
    def wrap_dscp(self) -> str:
        ...
        if apumd_cfg.version == 0:
            ...
            t_y = op_module.map_flt_apumd(t_x, t_table, t_table_grad, t_table_info)
            sess = get_sess()
            # GS, when r = 0
            GSs = []
        ...
        for ss in range(NSEL):
            ...
            for ll in range(nlayer_fit):
                ...
                if nc == 1:
                    ...
                    bbp += [''.join([bdc[ll][tt][sc * ncs * 0 + cc] for cc in range(ncs)] * numdata)]
    ...
    def wrap_lut(self) -> tuple[list[str], list[str], list[str]]:
        ...
        # std
        d = maps["dstd_inv"]
        d2 = np.zeros([ntype_max, 2], dtype=GLOBAL_NP_FLOAT_PRECISION)
...
def wrap(
    *,
    apumd_config: str | None = "apumd/config.npy",
    apumd_weight: str | None = "apumd/weight.npy",
    apumd_map: str | None = "apumd/map.npy",
    apumd_model: str | None = "apumd/model.pb",
    **kwargs: Any,
) -> None:
    wrapObj = Wrap(apumd_config, apumd_weight, apumd_map, apumd_model)
    wrapObj.wrap()
