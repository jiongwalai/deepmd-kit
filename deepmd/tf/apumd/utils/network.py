    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        if apumd_cfg.quantize_fitting_net:
            ...
            with tf.variable_scope("wxb", reuse=reuse):
                wxb = op_module.quantize_apumd(wxb, 1, NBIT_DATA_FL, NBIT_DATA_FL, -1)
                wxb = tf.ensure_shape(wxb, [None, outputs_size])
            # actfun
            if activation_fn is not None:
                # set activation function as tanh4
                y = op_module.tanh4_flt_apumd(wxb)
            else:
                y = wxb

            with tf.variable_scope("actfun", reuse=reuse):
                y = op_module.quantize_apumd(y, 1, NBIT_DATA_FL, NBIT_DATA_FL, -1)
