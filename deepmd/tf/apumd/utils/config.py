    ...
    def init_dscp(self, jdata: dict, jdata_parent: dict = {}) -> dict:
        ...
        if self.version == 1:
            ...
            if jdata["sel"] <= 128:
                ...
                jdata["NSTEP"] = 8 if self.device == "vu13p" else 16
            elif 160 < jdata["sel"] <= 192:
                jdata["NSTEP"] = 16 if self.device == "vu13p" else 32
            elif 192 < jdata["sel"] <= 224:
                jdata["NSTEP"] = 24 if self.device == "vu13p" else 48
            elif 224 < jdata["sel"] <= 256:
                jdata["NSTEP"] = 32 if self.device == "vu13p" else 64
            else:
