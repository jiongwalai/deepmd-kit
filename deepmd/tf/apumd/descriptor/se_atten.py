...
def build_op_descriptor() -> Callable:
    ...
    if apumd_cfg.quantize_descriptor:
        return op_module.prod_env_mat_a_mix_apumd_quantize
    else:
...
def descrpt2r4(inputs: tf.Tensor, atype: tf.Tensor) -> tf.Tensor:
    ...
    table = GLOBAL_NP_FLOAT_PRECISION(
        np.concatenate([apumd_cfg.map["s"][0], apumd_cfg.map["h"][0]], axis=1)
    )
    table_grad = GLOBAL_NP_FLOAT_PRECISION(
        np.concatenate(
            [apumd_cfg.map["s_grad"][0], apumd_cfg.map["h_grad"][0]],
            axis=1,
        )
    )
    ...
    s_h = op_module.map_flt_apumd(u, table, table_grad, table_info)
    ...
    with tf.variable_scope("s_s", reuse=True):
        s = op_module.flt_apumd(s)
        log.debug("#s_s: %s", s)
        s = tf.ensure_shape(s, [None, 1])

    with tf.variable_scope("h_s", reuse=True):
        h = op_module.flt_apumd(h)
        log.debug("#h_s: %s", h)
        h = tf.ensure_shape(h, [None, 1])
    # davg and dstd
    # davg = apumd_cfg.map["davg"]  # is_zero
    dstd_inv = apumd_cfg.map["dstd_inv"]
    ...
    s = op_module.mul_flt_apumd(std_inv_s, tf.reshape(s, [-1, NIDP]))
    h = op_module.mul_flt_apumd(std_inv_h, tf.reshape(h, [-1, NIDP]))
...
def filter_lower_R42GR(
    ...
    # copy
    inputs_reshape = op_module.flt_apumd(inputs_reshape)
    inputs_reshape = tf.ensure_shape(inputs_reshape, [None, 4])

    inputs_reshape, inputs_reshape2 = op_module.copy_flt_apumd(inputs_reshape)
    inputs_reshape = tf.ensure_shape(inputs_reshape, [None, 4])
    ...
    table_info = np.array([np.float64(v) for vs in table_info for v in vs])
    table_info = GLOBAL_NP_FLOAT_PRECISION(table_info)
    G = op_module.map_flt_apumd(s, table, table_grad, table_info)
    G = tf.ensure_shape(G, [None, 1, M1])
    with tf.variable_scope("g_s", reuse=True):
        G = op_module.flt_apumd(G)
        log.debug("#g_s: %s", G)
        G = tf.ensure_shape(G, [None, 1, M1])
    # t2G
    atype_expand = tf.reshape(atype, [-1, 1])
    ...
    with tf.variable_scope("g_t", reuse=True):
        two_embd = op_module.flt_apumd(two_embd)
        log.debug("#g_t: %s", two_embd)
        two_embd = tf.ensure_shape(two_embd, [None, M1])
    # G_s, G_t -> G
    G = tf.reshape(G, [-1, M1])
    G = op_module.mul_flt_apumd(G, two_embd)
    ...
    GR = op_module.matmul_flt2fix_apumd(
...
def filter_GR2D(xyz_scatter_1: tf.Tensor) -> tuple[tf.Tensor, tf.Tensor]:
    ...
    if apumd_cfg.quantize_descriptor:
        ...
        with tf.variable_scope("gr", reuse=True):
            xyz_scatter_1 = op_module.flt_apumd(xyz_scatter_1)
        ...
        result = op_module.matmul_flt_apumd(
        ...
        with tf.variable_scope("d", reuse=True):
            result = op_module.flt_apumd(result)
            log.debug("#d: %s", result)
            result = tf.ensure_shape(result, [None, M1 * M2])

        result = op_module.quantize_apumd(result, 0, NBIT_DATA_FL, NBIT_DATA_FL, -1)
        result = tf.ensure_shape(result, [None, M1 * M2])
    else:
