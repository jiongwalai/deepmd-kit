...
@ops.RegisterGradient("MapFltApumd")
def _MapFltApumdGrad(op: tf.Operation, grad: tf.Tensor) -> list[tf.Tensor | None]:
    x = op.inputs[0]
    table = op.inputs[1]
    ...
    M = shw[1] // 4
    #
    dydx = op_module.map_flt_apumd(x, table_grad, tf.zeros_like(table_grad), table_info)
    dydx = tf.ensure_shape(dydx, [N, D, M])
    # calculate
    dx = op_module.dotmul_flt_apumd(dydx, grad)
