...
@ops.RegisterGradient("MatmulFltApumd")
def _MatmulFltApumdGrad(op: tf.Operation, grad: tf.Tensor) -> list[tf.Tensor]:
    x = op.inputs[0]
    w = op.inputs[1]
    ...
    modew = (normw >> 4) & 15
    if modex:
        dx = op_module.matmul_flt2fix_apumd(grad, w_T, 23)
    else:
        dx = op_module.matmul_flt_apumd(grad, w_T, normx, normw)
    if modew:
        dw = op_module.matmul_flt2fix_apumd(x_T, grad, 23)
    else:
        dw = op_module.matmul_flt_apumd(x_T, grad, 1, normx)
