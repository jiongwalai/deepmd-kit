...
@ops.RegisterGradient("MatmulFlt2fixApumd")
def _MatmulFlt2fixApumdGrad(op: tf.Operation, grad: tf.Tensor) -> list[tf.Tensor]:
    x = op.inputs[0]
    w = op.inputs[1]
    ...
    dx = op_module.matmul_flt_apumd(grad, w_T, 1, 1)
    dw = op_module.matmul_flt_apumd(x_T, grad, 1, 1)
